...
import { useAuthenticatedChatStore, ChatMessage } from "@/store/authenticated-chat-store";
import { toast } from "sonner";
import { useModalStore } from "@/store/modal-store";
import { useRef } from "react";

export function useAuthenticatedChat(documentId: string) {
  const queryClient = useQueryClient();
  const { setMessages, addMessage, updateMessage, setLoading, setError, setTyping, preferredLanguage } = useAuthenticatedChatStore();
  const abortControllerRef = useRef<AbortController | null>(null);

  // Fetch initial messages
  const { isLoading: isFetchingHistory } = useQuery({
    queryKey: ["chat", documentId || "general"],
    queryFn: async () => {
