    ...
    return (
      <div className="max-w-4xl mx-auto py-16 px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <Card className="flex flex-col items-center p-8 text-center hover:border-primary/50 transition-colors">
            ...
            <Link href="/clients/new" className="w-full bg-primary text-primary-foreground py-2 rounded-md font-medium text-sm flex items-center justify-center gap-2">
          ...
          <Card className="flex flex-col items-center p-8 text-center hover:border-primary/50 transition-colors">
            ...
            <Link href="/matters/new" className="w-full bg-primary text-primary-foreground py-2 rounded-md font-medium text-sm flex items-center justify-center gap-2">
          ...
          <Card className="flex flex-col items-center p-8 text-center hover:border-primary/50 transition-colors">
            ...
            <Link href="/documents/upload" className="w-full bg-primary text-primary-foreground py-2 rounded-md font-medium text-sm flex items-center justify-center gap-2">
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-16">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-3">
          <Link href="/documents/upload" className="bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm font-medium shadow-sm hover:bg-primary/90">
            Upload Document
          </Link>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Link href="/matters" className="text-sm text-primary hover:underline">View All</Link>
          </div>
          <Card>
              ...
              {recentMatters && recentMatters.length > 0 ? recentMatters.map((matter: any) => (
                <div key={matter.id} className="flex items-center justify-between p-4 hover:bg-muted/30 transition-colors">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{matter.clients?.client_name || 'Unknown Client'}</p>
                  </div>
                  <Badge variant="outline" className={matter.status === "Active" ? "bg-green-500/10 text-green-600" : "bg-orange-500/10 text-orange-600"}>
                    {matter.status}
                  </Badge>
                </div>
              )) : (
                <div className="p-4 text-center text-muted-foreground">No recent matters found.</div>
              )}
            </div>
          </Card>
        </div>
              ...
              {recentDocuments && recentDocuments.length > 0 ? recentDocuments.map((doc: any) => (
                <div key={doc.id} className="flex items-center justify-between p-4 hover:bg-muted/30 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${doc.overall_risk_level === 'High' ? 'bg-destructive' : doc.overall_risk_level === 'Medium' ? 'bg-orange-500' : 'bg-green-500'}`} />
                    <div>
                ...
                <div className="p-4 text-center text-muted-foreground">No recent documents found.</div>
              )}
            </div>
          </Card>
        </div>
