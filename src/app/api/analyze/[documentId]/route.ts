  ...
  try {
    ...
    if (isDevelopmentAnalysis) {
      if (process.env.NODE_ENV === "development") {
        // Missing AI or S3 keys in development. Simulating analysis...
      }
    ...
    try {
      ...
      if (!document.storage_key) {
        console.warn("Document storage_key is missing for document ID:", documentId);
      ...
      try {
        ...
        await supabaseFallback.from("documents").update({ 
          status: "failed",
          error_message: error.message || "Unknown processing error",
          processed_at: new Date().toISOString()
        }).eq("id", documentId);
      } catch (dbError) {
        console.error("Secondary error updating document status in DB:", dbError);
      }
