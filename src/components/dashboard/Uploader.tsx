...
export function Uploader() {
  ...
  return (
    <Card 
      {...getRootProps()} 
      className={cn(
        "border-2 border-dashed bg-muted/10 transition-colors duration-200 cursor-pointer rounded-[24px] overflow-hidden",
        isDragActive ? "border-primary bg-primary/5" : "hover:bg-muted/20 hover:border-primary/50",
        ...
        <AnimatePresence mode="wait">
          {status === "idle" && (
            <motion.div key="idle" initial="hidden" animate="visible" exit="hidden" variants={fadeUp} className="flex flex-col items-center w-full max-w-md mx-auto">
              <div className="h-24 w-24 bg-primary/10 rounded-full flex items-center justify-center mb-8 shadow-sm transition-transform duration-300 group-hover:scale-105">
          ...
          {status === "success" && (analysisStatus === "idle" || analysisStatus === "completed") && file && (
            <motion.div key="success" initial="hidden" animate="visible" exit="hidden" variants={fadeUp} className="w-full max-w-md flex flex-col items-center mx-auto">
              ...
              <button 
                ...
                className={buttonVariants({ variant: "outline", className: "w-full h-12 rounded-[12px]" })}
              >
                Upload Another
              </button>
            </motion.div>
          ...
          {analysisStatus === "processing" && (
            <motion.div key="processing" initial="hidden" animate="visible" exit="hidden" variants={fadeUp} className="w-full max-w-md flex flex-col items-center mx-auto">
              <div className="relative mb-6">
                ...
                <Loader2 className="h-14 w-14 text-blue-500 animate-spin relative z-10" />
              </div>
          ...
          {analysisStatus === "failed" && (
            <motion.div key="analysis-failed" initial="hidden" animate="visible" exit="hidden" variants={fadeUp} className="w-full max-w-md flex flex-col items-center mx-auto">
              ...
              <p className="text-destructive mb-8 text-base font-body text-center">There was an error generating AI insights. Please try again.</p>
              <button 
                onClick={(e) => { 
                  e.stopPropagation(); 
                  const docId = useUploadStore.getState().documentId;
          ...
          {status === "error" && (
            <motion.div key="error" initial="hidden" animate="visible" exit="hidden" variants={fadeUp} className="w-full max-w-md flex flex-col items-center mx-auto">
              ...
              <p className="text-destructive mb-8 text-base font-body">There was an error uploading your file. Please check your connection and try again.</p>
              <button 
                onClick={(e) => { e.stopPropagation(); reset(); }}
                className={buttonVariants({ variant: "outline", className: "h-12 px-8 rounded-[12px]" })}
              >
                Try Again
              </button>
            </motion.div>
